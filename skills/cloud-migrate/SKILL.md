...
**多模块项目：** 对每个子模块单独启动一个 Agent E 实例（可并行），每个实例只处理一个模块的数据。
...
模块标识：[MODULE_KEY]（多模块时填写，单模块留空）
子模块路径：[MODULE_PATH]（多模块时为相对仓库根目录的路径）
项目类型：[TOMCAT_WAR / DUBBO_JAR / SPRINGBOOT / FRONTEND]
应用名：[APP_NAME]
...
批量模式中，如果多个仓库使用同一套平台地址，可以从一个项目的 `.cloud-migrate.json` 复制到其他项目：
```bash
...
d['app_name'] = name
d['updated_at'] = datetime.datetime.now().isoformat()
with open('$dir/.cloud-migrate.json', 'w') as f:
    json.dump(d, f, indent=2, ensure_ascii=False)
print(f'✅ {name}')
"
  fi
...
> 批量复制时只复制平台地址，`app_name` 自动设为目录名，`modules` 不复制（各仓库的子模块结构不同，需单独配置）。
